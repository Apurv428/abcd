    ...
    if (signInError) {
      if (
        ...
        const { data: signUpCheck } = await supabase.auth.signUp({
          email,
          password: "check_existence_only_placeholder_99!",
        });
        // If user was "created" that means the email doesn't exist
        if (signUpCheck?.user?.identities?.length === 0 || signUpCheck?.user?.confirmation_sent_at) {
          // Email doesn't exist — redirect to register
          router.push(`/register?email=${encodeURIComponent(email)}&msg=no_account`);
          return;
        }
    ...
    >
      ...
      >
        ...
        <p style={{ textAlign: "center", marginTop: "24px", fontSize: "0.85rem", color: "var(--text-secondary)" }}>
          ...
          <Link href="/register" style={{ color: "var(--accent-teal)", textDecoration: "none", fontWeight: 600 }}>
            Create one
          </Link>
        </p>
      </div>
